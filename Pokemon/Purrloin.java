public class Purrloin extends Pokemon{
	{
		name = "Purrloin";
		bHP = 4;
		bAtk = 5;
		bDef = 4;
		bSpAtk = 5;
		bSpDef = 4;
		bSpd = 7;
		type1 = "Dark";
		type2 = "";
		abilities = new String[]{"Limber", "Unburden", "Cruelty", "Prankster", "Vicious"};
		abilityTypeCount = new int[] {2,2,1};
		next = new String[]{"Liepard"};
		nextReq = new String[]{"20"};
		height = "1\'/4\"";
		size = "Small";
		weight = 22.3;
		WC = 1;
		maleRatio = 50.0;
		femaleRatio = 50.0;
		egg1 = "Field";
		egg2 = "";
		habitats = new String[]{"Forest", "Grassland", "Rainforest"};
		overland = 5;
		swim = 2;
		levitate = 0;
		burrow = 0;
		lJump = 1 ;
		hJump = 1;
		power = 1;
		natureWalk = new String[]{"Grassland", "Urban"};
		underdog = true;
		other = new String[]{"Darkvision","Stealth"};
		athl = 2;
		acro = 3;
		combat = 2;
		stealth = 3;
		percep = 2;
		focus = 2;
		athlBonus = 0;
		acroBonus = 1;
		combatBonus = 0;
		stealthBonus = 2;
		percepBonus = 0;
		focusBonus = 0;
		movesLevels = new int[]{1, 3, 6, 10, 12, 15, 19, 21, 24, 28, 30, 33, 37, 39, 42, 46, 49};
		moves = new String[]{"Scratch", "Growl", "Assist", "Sand Attack", "Fury Swipes", "Pursuit", "Torment", "Fake Out", "Hone Claws", "Assurance", "Slash", "Captivate", "Night Slash", "Snatch", "Nasty Plot", "Sucker Punch", "Play Rough"};
		TMs = new String[]{"Toxic", "Hidden Power", "Sunny Day", "Taunt", "Protect", "Rain Dance", "Frustration", "Return", "Shadow Ball", "Double Team", "Aerial Ace", "Torment", "Facade", "Rest", "Attract", "Thief", "Round", "Echoed Voice", "Embargo", "Shadow Claw", "Payback", "Thunder", "Thunder Wave", "Psych Up", "Dream Eater", "Grass Knot", "Swagger", "Sleep Talk", "Substitute", "Snarl", "Dark Pulse", "Confide"};
		eggMoves = new String[]{"Charm", "Copycat", "Covet", "Encore", "Feint Attack", "Fake Tears", "Foul Play", "Pay Day", "Yawn"};
		tutorMoves = new String[]{"Covet", "Foul Play", "Gunk Shot", "Hyper Voice", "Iron Tail", "Knock Off", "Role Play", "Seed Bomb", "Snatch", "Snore", "Spite", "Trick"};
		naturalMoves = new String[]{};
	}
		Purrloin(){

		super();
		}
}
public class Miltank extends Pokemon{
	{
		name = "Miltank";
		bHP = 10;
		bAtk = 8;
		bDef = 11;
		bSpAtk = 4;
		bSpDef = 7;
		bSpd = 10;
		type1 = "Normal";
		type2 = "";
		abilities = new String[]{"Thick Fat", "Scrappy", "Lunchbox", "Sap Sipper", "Run Up"};
		abilityTypeCount = new int[] {2,2,1};
		height = "3\'/11\"";
		weight = 166.4;
		WC = 4;
		maleRatio = 0.0;
		femaleRatio = 100.0;
		egg1 = "Field";
		egg2 = "";
		habitats = new String[]{"Grassland"};
		overland = 5;
		swim = 2;
		levitate = 0;
		burrow = 0;
		lJump = 1 ;
		hJump = 1;
		power = 5;
		underdog = false;
		other = new String[]{"Milk Collection"};
		athl = 3;
		acro = 2;
		combat = 2;
		stealth = 2;
		percep = 2;
		focus = 2;
		athlBonus = 1;
		acroBonus = 0;
		combatBonus = 0;
		stealthBonus = 0;
		percepBonus = 2;
		focusBonus = 2;
		movesLevels = new int[]{1, 3, 5, 8, 11, 15, 19, 24, 29, 35, 41, 48, 55};
		moves = new String[]{"Tackle", "Growl", "Defense Curl", "Stomp", "Milk Drink", "Bide", "Rollout", "Body Slam", "Zen Headbutt", "Captivate", "Gyro Ball", "Heal Bell", "Wake Up Slap"};
		TMs = new String[]{"Surf", "Strength", "Toxic", "Hidden Power", "Sunny Day", "Ice Beam", "Blizzard", "Hyper Beam", "Protect", "Rain Dance", "Frustration", "Solar Beam", "Thunderbolt", "Thunder", "Earthquake", "Return", "Shadow Ball", "Brick Break", "Double Team", "Sandstorm", "Rock Tomb", "Facade", "Rest", "Attract", "Round", "Echoed Voice", "Focus Blast", "Fling", "Retaliate", "Giga Impact", "Thunder", "Thunder Wave", "Gyro Ball", "Psych Up", "Bulldoze", "Rock Slide", "Swagger", "Sleep Talk", "Substitute", "Rock Smash", "Confide"};
		eggMoves = new String[]{"Belch", "Curse", "Dizzy Punch", "Endure", "Hammer Arm", "Heart Stamp", "Helping Hand", "Natural Gift", "Present", "Psych Up", "Punishment", "Reversal", "Seismic Toss", "Sleep Talk"};
		tutorMoves = new String[]{"After You", "Counter", "Block", "Dynamic Punch", "Fire Punch", "Focus Punch", "Heal Bell", "Helping Hand", "Ice Punch", "Icy Wind", "Iron Head", "Iron Tail", "Mega Kick", "Mega Punch", "Metronome", "Rollout", "Shock Wave", "Sleep Talk", "Snore", "Stealth Rock", "Thunder Punch", "Water Pulse", "Work Up", "Zen Headbutt", "Headbutt"};
		naturalMoves = new String[]{};
	}
		Miltank(){

		super();
		}
}
public class Togekiss extends Pokemon{
	{
		name = "Togekiss";
		bHP = 9;
		bAtk = 5;
		bDef = 10;
		bSpAtk = 12;
		bSpDef = 12;
		bSpd = 8;
		type1 = "Fairy";
		type2 = "Flying";
		abilities = new String[]{"Hustle", "Serene Grace", "Cute Charm", "Gentle Vibe", "Super Luck"};
		abilityTypeCount = new int[] {2,2,1};
		previous = new Togetic();
		prevReq = 20;
		height = "4\'/11\"";
		weight = 83.8;
		WC = 3;
		maleRatio = 87.5;
		femaleRatio = 12.5;
		egg1 = "Flying";
		egg2 = "Fairy";
		overland = 3;
		swim = 3;
		levitate = 0;
		burrow = 0;
		lJump = 2 ;
		hJump = 2;
		power = 6;
		underdog = false;
		other = new String[]{"Guster"};
		athl = 4;
		acro = 3;
		combat = 3;
		stealth = 2;
		percep = 4;
		focus = 4;
		athlBonus = 2;
		acroBonus = 0;
		combatBonus = 0;
		stealthBonus = 0;
		percepBonus = 1;
		focusBonus = 3;
		movesLevels = new int[]{5, 9, 13, 14, 17, 21, 29, 33, 37, 41, 45, 49, 53};
		moves = new String[]{"Metronome", "Sweet Kiss", "Yawn", "Fairy Wind", "Encore", "Follow Me", "Wish", "Ancient Power", "Safeguard", "Baton Pass", "DoubleEdge", "Last Resort", "After You"};
		TMs = new String[]{"Fly", "Psyshock", "Toxic", "Hidden Power", "Sunny Day", "Hyper Beam", "Light Screen", "Protect", "Rain Dance", "Roost", "Safeguard", "Frustration", "Solar Beam", "Return", "Psychic", "Shadow Ball", "Brick Break", "Double Team", "Reflect", "Flamethrower", "Fire Blast", "Aerial Ace", "Facade", "Rest", "Attract", "Round", "Echoed Voice", "Steel Wing", "Fling", "Incinerate", "Retaliate", "Giga Impact", "Flash", "Thunder", "Thunder Wave", "Psych Up", "Dream Eater", "Grass Knot", "Swagger", "Sleep Talk", "Substitute", "Rock Smash", "Dazzling Gleam", "Confide"};
		tutorMoves = new String[]{"Air Cutter", "After You", "Ancient Power", "Body Slam", "Slam", "Counter", "Covet", "Defense Curl", "Defog", "Drain Punch", "Endeavor", "Focus Punch", "Heal Bell", "Heat Wave", "Hyper Voice", "Last Resort", "Magic Coat", "Mega Kick", "Mega Punch", "Mimic", "Natural Gift", "Ominous Wind", "Pluck", "Roost", "Role Play", "Rollout", "Secret Power", "Seismic Toss", "Signal Beam", "Silver Wind", "Sleep Talk", "Snore", "Steel Wing", "Swift", "Tailwind", "Telekinesis", "Trick", "Twister", "Uproar", "Water Pulse", "Work Up", "Zen Headbutt", "Headbutt"};
		naturalMoves = new String[]{"After You"};
	}
		Togekiss(){

		super();
		}
}
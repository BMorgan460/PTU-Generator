public class Froakie extends Pokemon{
	{
		name = "Froakie";
		bHP = 4;
		bAtk = 6;
		bDef = 4;
		bSpAtk = 6;
		bSpDef = 4;
		bSpd = 7;
		type1 = "Water";
		type2 = "";
		abilities = new String[]{"Mud Dweller", "Torrent", "Protean", "Hydration", "Wash Away"};
		abilityTypeCount = new int[] {2,2,1};
		next = new String[]{"Frogadier", "Greninja"};
		nextReq = new String[]{"30"};
		height = "1\'/0\"";
		size = "Small";
		weight = 15.4;
		WC = 1;
		maleRatio = 87.5;
		femaleRatio = 12.5;
		egg1 = "Water";
		egg2 = "Average";
		overland = 5;
		swim = 4;
		levitate = 0;
		burrow = 0;
		lJump = 2 ;
		hJump = 2;
		power = 2;
		natureWalk = new String[]{"Wetlands"};
		underdog = true;
		other = new String[]{"Fountain","Gilled","Wallclimber"};
		athl = 2;
		acro = 3;
		combat = 2;
		stealth = 3;
		percep = 2;
		focus = 2;
		athlBonus = 0;
		acroBonus = 2;
		combatBonus = 0;
		stealthBonus = 2;
		percepBonus = 1;
		focusBonus = 0;
		movesLevels = new int[]{1, 1, 5, 8, 10, 14, 18, 21, 25, 29, 35, 39, 43, 48};
		moves = new String[]{"Pound", "Growl", "Bubble", "Quick Attack", "Lick", "Water Pulse", "Smokescreen", "Round", "Fling", "Smack Down", "Substitute", "Bounce", "Double Team", "Hydro Pump"};
		TMs = new String[]{"Work Up", "Toxic", "Hidden Power", "Taunt", "Ice Beam", "Blizzard", "Protect", "Rain Dance", "Frustration", "Smack Down", "Return", "Double Team", "Rock Tomb", "Aerial Ace", "Facade", "Rest", "Attract", "Thief", "Round", "Echoed Voice", "Scald", "Fling", "Acrobatics", "Rock Slide", "Grass Knot", "Swagger", "Sleep Talk", "Substitute", "Surf", "Waterfall", "Confide"};
		eggMoves = new String[]{"Bestow", "Camouflage", "Mind Reader", "Mud Sport", "Toxic Spikes", "Spikes", "Water Sport"};
		tutorMoves = new String[]{"Bounce", "Icy Wind", "Role Play", "Snatch", "Snore", "Spite", "Water Pledge", "Water Pulse"};
		naturalMoves = new String[]{};
	}
		Froakie(){

		super();
		}
}
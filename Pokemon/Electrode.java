public class Electrode extends Pokemon{
{
		name = "Electrode";
		bHP = 6;
		bAtk = 5;
		bDef = 7;
		bSpAtk = 8;
		bSpDef = 8;
		bSpd = 14;
		type1 = "Electric";
		type2 = "";
		abilities = new String[]{"Static", "Soundproof", "Speed Boost", "Plus", "Aftermath"};
		abilityTypeCount = new int[] {2,2,1};
		previous = new Voltorb();
		prevReq = 30;
		height = "3\'/11\"";
		weight = 146.8;
		WC = 4;
		maleRatio = -1;
		femaleRatio = -1;
		egg1 = "Mineral";
		egg2 = "Diet";
		habitats = new String[]{"Urban", "Capability", "List"};
		overland = 8;
		swim = 3;
		levitate = 0;
		sky = 0;
		burrow = 0;
		lJump = 1;
		hJump = 1;
		power = 4;
		underdog = false;
		athl = 4;
		acro = 3;
		combat = 1;
		stealth = 4;
		percep = 3;
		focus = 3;
		athlBonus = 1;
		acroBonus = 1;
		combatBonus = 0;
		stealthBonus = 2;
		percepBonus = 0;
		focusBonus = 1;
		movesLevels = new int[]{4, 6, 9, 11, 13, 16, 20, 22, 26, 29, 36, 41, 47, 54, 58};
		moves = new String[]{"Sonic Boom", "Eerie Impulse", "Spark", "Rollout", "Screech", "Charge Beam", "Swift", "Electro Ball", "SelfDestruct", "Light Screen", "Magnet Rise", "Discharge", "Explosion", "Gyro Ball", "Mirror Coat"};
		TMs = new String[]{"Toxic", "Hidden Power", "Taunt", "Hyper Beam", "Light Screen", "Protect", "Rain Dance", "Frustration", "Thunderbolt", "Thunder", "Return", "Double Team", "Torment", "Facade", "Rest", "Thief", "Round", "Charge Beam", "Explosion", "Giga Impact", "Flash", "Volt Switch", "Thunder Wave", "Gyro Ball", "Swagger", "Sleep Talk", "Substitute", "Wild Charge"};
		tutorMoves = new String[]{"Foul Play", "Magic Coat", "Magnet Rise", "Magnetic Flux", "Role Play", "Rollout", "Shock Wave", "Signal Beam", "Sleep Talk", "Snore", "Sucker Punch", "Swift"};
		naturalMoves = new String[]{"Magnetic Flux"};
}
		Electrode(){

		super();
		}
}
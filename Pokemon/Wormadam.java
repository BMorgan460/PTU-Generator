public class Wormadam extends Pokemon{
{
		name = "Wormadam";
		bHP = 7;
		bAtk = 8;
		bDef = 11;
		bSpAtk = 6;
		bSpDef = 9;
		bSpd = 3;
		type1 = "Bug";
		type2 = "";
		abilities = new String[]{"Anticipation", "Overcoat", "Adaptability", "Tolerance", "Grass Pelt"};
		abilityTypeCount = new int[] {1,3,1};
		//previous = new Burmy();
		prevReq = 20;
		height = "1\'/8\"";
		size = "Small";
		weight = 14.3;
		WC = 1;
		maleRatio = 0;
		femaleRatio = 100;
		egg1 = "Bug";
		egg2 = "Diet";
		habitats = new String[]{"Desert", "Mountain", "Capability"};
		overland = 4;
		swim = 1;
		levitate = 0;
		sky = 0;
		burrow = 0;
		lJump = 1;
		hJump = 2;
		power = 2;
		underdog = false;
		athl = 2;
		acro = 3;
		combat = 2;
		stealth = 4;
		percep = 4;
		focus = 3;
		athlBonus = 0;
		acroBonus = 0;
		combatBonus = 0;
		stealthBonus = 0;
		percepBonus = 0;
		focusBonus = 0;
		movesLevels = new int[]{10, 15, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47};
		moves = new String[]{"Protect", "Bug Bite", "Hidden Power", "Confusion", "Rock Blast", "Harden", "Psybeam", "Captivate", "Flail", "Attract", "Psychic", "Fissure"};
		TMs = new String[]{"Toxic", "Venoshock", "Hidden Power", "Sunny Day", "Hyper Beam", "Protect", "Rain Dance", "Safeguard", "Frustration", "Solar Beam", "Return", "Psychic", "Shadow Ball", "Double Team", "Facade", "Rest", "Attract", "Thief", "Round", "Energy Ball", "Giga Impact", "Flash", "Struggle Bug", "Psych Up", "Dream Eater", "Grass Knot", "Swagger", "Sleep Talk", "Substitute"};
		tutorMoves = new String[]{"Bug Bite", "Bite", "Earth Power", "Electroweb", "Endeavor", "Gunk Shot", "Natural Gift", "Signal Beam", "Skill Swap", "Snore", "Stealth Rock", "String Shot", "Sucker Punch", "Uproar"};
		naturalMoves = new String[]{};
}
		Wormadam(){

		super();
		}
}
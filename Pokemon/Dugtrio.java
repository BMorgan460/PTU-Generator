public class Dugtrio extends Pokemon{
{
		name = "Dugtrio";
		bHP = 4;
		bAtk = 8;
		bDef = 5;
		bSpAtk = 5;
		bSpDef = 7;
		bSpd = 12;
		type1 = "Ground";
		type2 = "";
		abilities = new String[]{"Sand Veil", "Arena Trap", "Sand Force", "Polycephaly", "Away"};
		abilityTypeCount = new int[] {2,2,1};
		previous = new Diglett();
		prevReq = 25;
		height = "2\'/4\"";
		size = "Small";
		weight = 73.4;
		WC = 3;
		maleRatio = 50;
		femaleRatio = 50;
		egg1 = "Field";
		egg2 = "Diet";
		habitats = new String[]{"Cave", "Grassland", "Forest"};
		overland = 2;
		swim = 0;
		levitate = 0;
		sky = 0;
		burrow = 9;
		lJump = 0;
		hJump = 0;
		power = 4;
		underdog = false;
		athl = 3;
		acro = 3;
		combat = 2;
		stealth = 5;
		percep = 4;
		focus = 4;
		athlBonus = 0;
		acroBonus = 0;
		combatBonus = 0;
		stealthBonus = 2;
		percepBonus = 1;
		focusBonus = 1;
		movesLevels = new int[]{4, 7, 12, 15, 18, 23, 26, 28, 33, 40, 45, 50, 57};
		moves = new String[]{"Growl", "Astonish", "MudSlap", "Magnitude", "Bulldoze", "Sucker Punch", "Sand Tomb", "Mud Bomb", "Earth Power", "Dig", "Slash", "Earthquake", "Fissure"};
		TMs = new String[]{"Hone Claws", "Toxic", "Hidden Power", "Sunny Day", "Hyper Beam", "Protect", "Frustration", "Earthquake", "Return", "Dig", "Double Team", "Sludge Wave", "Sludge Bomb", "Sandstorm", "Rock Tomb", "Aerial Ace", "Facade", "Rest", "Attract", "Thief", "Round", "Echoed Voice", "Shadow Claw", "Giga Impact", "Stone Edge", "Bulldoze", "Rock Slide", "Swagger", "Sleep Talk", "Substitute", "Rock Smash"};
		tutorMoves = new String[]{"Body Slam", "Slam", "Earth Power", "Fury Swipes", "Night Slash", "Sleep Talk", "Snore", "Stealth Rock", "Sucker Punch", "Uproar"};
		naturalMoves = new String[]{"Night Slash", "Slash", "Rototiller"};
}
		Dugtrio(){

		super();
		}
}
public class Mareep extends Pokemon{
	{
		name = "Mareep";
		bHP = 6;
		bAtk = 4;
		bDef = 4;
		bSpAtk = 7;
		bSpDef = 5;
		bSpd = 4;
		type1 = "Electric";
		type2 = "";
		abilities = new String[]{"Static", "Fluffy Charge", "Sap Sipper", "Sequence", "Fur Coat"};
		abilityTypeCount = new int[] {1,3,1};
		next = new String[]{"Flaaffy", "Ampharos"};
		nextReq = new String[]{"30"};
		height = "2\'/0\"";
		size = "Small";
		weight = 17.2;
		WC = 1;
		maleRatio = 50.0;
		femaleRatio = 50.0;
		egg1 = "Monster";
		egg2 = "Field";
		habitats = new String[]{"Grassland"};
		overland = 4;
		swim = 2;
		levitate = 0;
		burrow = 0;
		lJump = 1 ;
		hJump = 1;
		power = 1;
		natureWalk = new String[]{"Grassland"};
		underdog = true;
		other = new String[]{"Glow","Zapper"};
		athl = 2;
		acro = 3;
		combat = 1;
		stealth = 3;
		percep = 2;
		focus = 2;
		athlBonus = 0;
		acroBonus = 0;
		combatBonus = 1;
		stealthBonus = 1;
		percepBonus = 0;
		focusBonus = 0;
		movesLevels = new int[]{1, 1, 4, 8, 11, 15, 18, 22, 25, 29, 32, 36, 39, 43, 46};
		moves = new String[]{"Tackle", "Growl", "Thunder Wave", "Thunder Shock", "Cotton Spore", "Charge", "Take Down", "Electro Ball", "Confuse Ray", "Power Gem", "Discharge", "Cotton Guard", "Signal Beam", "Light Screen", "Thunder"};
		TMs = new String[]{"Toxic", "Hidden Power", "Light Screen", "Protect", "Rain Dance", "Safeguard", "Frustration", "Thunderbolt", "Thunder", "Return", "Double Team", "Facade", "Rest", "Attract", "Round", "Echoed Voice", "Charge", "Charge Beam", "Thunder", "Thunder Wave", "Swagger", "Sleep Talk", "Substitute", "Wild Charge", "Confide"};
		eggMoves = new String[]{"After You", "Agility", "Body Slam", "Slam", "Charge", "Eerie Impulse", "Electric Terrain", "Flatter", "Iron Tail", "Odor Sleuth", "Reflect", "Safeguard", "Sand Attack", "Screech", "Take Down"};
		tutorMoves = new String[]{"After You", "Defense Curl", "Electroweb", "Heal Bell", "Iron Tail", "Magnet Rise", "Shock Wave", "Signal Beam", "Snore", "Swift"};
		naturalMoves = new String[]{};
	}
		Mareep(){

		super();
		}
}
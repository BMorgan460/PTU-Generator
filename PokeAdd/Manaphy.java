public class Manaphy extends Pokemon{
	{
		name = "Manaphy";
		bHP = 10;
		bAtk = 10;
		bDef = 10;
		bSpAtk = 10;
		bSpDef = 10;
		bSpd = 10;
		type1 = "Water";
		type2 = "";
		abilities = new String[]{"Hydration", "Rain Dish", "Swift Swim", "Damp", "Absorb Force"};
		abilityTypeCount = new int[] {2,2,1};
		height = "1\'/4\"";
		size = "Small";
		weight = 3.1;
		WC = 1;
		maleRatio = -1;
		femaleRatio = -1;
		egg1 = "Water";
		egg2 = " ";
		habitats = new String[]{"Ocean"};
		overland = 5;
		swim = 8;
		levitate = 0;
		burrow = 0;
		lJump = 2 ;
		hJump = 2;
		power = 2;
		natureWalk = new String[]{"Ocean"};
		underdog = false;
		other = new String[]{"Amorphous","Amorphous","Fountain","Gilled","Telepath"};
		athl = 4;
		acro = 5;
		combat = 3;
		stealth = 4;
		percep = 4;
		focus = 4;
		athlBonus = 2;
		acroBonus = 2;
		combatBonus = 0;
		stealthBonus = 3;
		percepBonus = 2;
		focusBonus = 0;
		movesLevels = new int[]{1, 1, 1, 9, 16, 24, 31, 39, 46, 54, 61, 69, 76};
		moves = new String[]{"Bubble", "Water Sport", "Tail Glow", "Charm", "Supersonic", "Bubble Beam", "Acid Armor", "Whirlpool", "Water Pulse", "Aqua Ring", "Dive", "Rain Dance", "Heart Swap"};
		TMs = new String[]{"Calm Mind", "Toxic", "Hail", "Hidden Power", "Ice Beam", "Blizzard", "Hyper Beam", "Light Screen", "Protect", "Rain Dance", "Safeguard", "Frustration", "Return", "Psychic", "Shadow Ball", "Double Team", "Reflect", "Facade", "Rest", "Round", "Energy Ball", "Scald", "Fling", "Giga Impact", "Psych Up", "Grass Knot", "Swagger", "Sleep Talk", "Substitute", "Surf", "Waterfall", "Dazzling Gleam", "Confide"};
		tutorMoves = new String[]{"Ancient Power", "Aqua Ring", "Bounce", "Brine", "Covet", "Dive", "Heal Bell", "Helping Hand", "Icy Wind", "Knock Off", "Last Resort", "Liquidation", "Signal Beam", "Skill Swap", "Snore", "Swift", "Uproar", "Water Pulse", "Whirlpool"};
		naturalMoves = new String[]{};
	}
		Manaphy(){

		super();
		}
}
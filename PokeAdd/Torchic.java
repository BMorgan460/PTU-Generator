public class Torchic extends Pokemon{
	{
		name = "Torchic";
		bHP = 5;
		bAtk = 6;
		bDef = 4;
		bSpAtk = 7;
		bSpDef = 5;
		bSpd = 5;
		type1 = "Fire";
		type2 = "";
		abilities = new String[]{"Blaze", "Klutz", "Speed Boost", "Tangled Feet", "Moxie"};
		abilityTypeCount = new int[] {2,2,1};
		next = new String[]{"Combusken", "Blaziken"};
		nextReq = new String[]{"30"};
		height = "1\'/4\"";
		size = "Small";
		weight = 5.5;
		WC = 1;
		maleRatio = 87.5;
		femaleRatio = 12.5;
		egg1 = "Field";
		egg2 = "";
		habitats = new String[]{"Grassland"};
		overland = 6;
		swim = 3;
		levitate = 0;
		burrow = 0;
		lJump = 1 ;
		hJump = 2;
		power = 1;
		natureWalk = new String[]{"Grassland"};
		underdog = true;
		other = new String[]{"Heater"};
		athl = 2;
		acro = 3;
		combat = 2;
		stealth = 3;
		percep = 2;
		focus = 1;
		athlBonus = 0;
		acroBonus = 3;
		combatBonus = 0;
		stealthBonus = 0;
		percepBonus = 0;
		focusBonus = 0;
		movesLevels = new int[]{1, 1, 5, 10, 14, 19, 23, 28, 32, 37, 41, 46};
		moves = new String[]{"Growl", "Scratch", "Ember", "Sand Attack", "Peck", "Fire Spin", "Quick Attack", "Flame Burst", "Focus Energy", "Slash", "Mirror Move", "Flamethrower"};
		TMs = new String[]{"Work Up", "Toxic", "Hidden Power", "Sunny Day", "Protect", "Frustration", "Return", "Double Team", "Flamethrower", "Fire Blast", "Rock Tomb", "Aerial Ace", "Facade", "Flame Charge", "Rest", "Attract", "Round", "Echoed Voice", "Overheat", "Shadow Claw", "Swords Dance", "Rock Slide", "Swagger", "Sleep Talk", "Substitute", "Confide"};
		eggMoves = new String[]{"Agility", "Baton Pass", "Counter", "Crush Claw", "Curse", "Endure", "Feather Dance", "Feint", "Flame Burst", "Last Resort", "Low Kick", "Night Slash", "Slash", "Reversal", "Rock Slide", "Smelling Salts", "Swagger"};
		tutorMoves = new String[]{"Body Slam", "Slam", "Bounce", "Defog", "Fire Pledge", "Heat Wave", "Helping Hand", "Last Resort", "Low Kick", "Mega Kick", "Mega Punch", "Seismic Toss", "Snore", "Swift"};
		naturalMoves = new String[]{};
	}
		Torchic(){

		super();
		}
}
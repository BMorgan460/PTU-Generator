public class Sentret extends Pokemon{
	{
		name = "Sentret";
		bHP = 4;
		bAtk = 5;
		bDef = 3;
		bSpAtk = 4;
		bSpDef = 5;
		bSpd = 2;
		type1 = "Normal";
		type2 = "";
		abilities = new String[]{"Run Away", "Keen Eye", "Frisk", "Perception", "Dodge"};
		abilityTypeCount = new int[] {2,2,1};
		next = new String[]{"Furret"};
		nextReq = new String[]{"15"};
		height = "2\'/7\"";
		size = "Small";
		weight = 13.2;
		WC = 1;
		maleRatio = 50.0;
		femaleRatio = 50.0;
		egg1 = "Field";
		egg2 = "";
		overland = 4;
		swim = 3;
		levitate = 0;
		burrow = 0;
		lJump = 3 ;
		hJump = 3;
		power = 1;
		natureWalk = new String[]{"Forest", "Grassland"};
		underdog = true;
		other = new String[]{};
		athl = 1;
		acro = 3;
		combat = 1;
		stealth = 3;
		percep = 4;
		focus = 2;
		athlBonus = 0;
		acroBonus = 0;
		combatBonus = 0;
		stealthBonus = 0;
		percepBonus = 0;
		focusBonus = 0;
		movesLevels = new int[]{1, 1, 4, 7, 13, 16, 19, 25, 28, 31, 36, 39, 42, 47};
		moves = new String[]{"Foresight", "Scratch", "Defense Curl", "Quick Attack", "Fury Swipes", "Helping Hand", "Follow Me", "Slam", "Rest", "Sucker Punch", "Amnesia", "Baton Pass", "Me First", "Hyper Voice"};
		TMs = new String[]{"Work Up", "Toxic", "Hidden Power", "Sunny Day", "Ice Beam", "Protect", "Rain Dance", "Frustration", "Solar Beam", "Thunderbolt", "Return", "Shadow Ball", "Brick Break", "Double Team", "Flamethrower", "Facade", "Rest", "Attract", "Thief", "Round", "Echoed Voice", "Fling", "Charge", "Charge Beam", "Brutal Swing", "Shadow Claw", "Grass Knot", "Swagger", "Sleep Talk", "Substitute", "Surf", "Confide"};
		eggMoves = new String[]{"Assist", "Captivate", "Charm", "Covet", "Focus Energy", "Iron Tail", "Last Resort", "Natural Gift", "Pursuit", "Reversal", "Slash", "Trick"};
		tutorMoves = new String[]{"Aqua Tail", "Body Slam", "Slam", "Covet", "Defense Curl", "Dynamic Punch", "Fire Punch", "Focus Punch", "Fury Cutter", "Helping Hand", "Hyper Voice", "Ice Punch", "Iron Tail", "Knock Off", "Last Resort", "Rollout", "Shock Wave", "Snore", "Sucker Punch", "Super Fang", "Swift", "Thunder Punch", "Trick", "Water Pulse", "Uproar"};
		naturalMoves = new String[]{};
	}
		Sentret(){

		super();
		}
}